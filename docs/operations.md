...
`granite-data-quality-hub` is centered on data engineering work in C++. The technical target is: Build a C++ toolkit that studies quality behavior through append-only fixtures, with checkpoint recovery checks and offline replay mode.
...
The score is deterministic. The current threshold is 172. Risk has a penalty of 7, latency has a penalty of 3, and weight contributes a bonus of 5.
